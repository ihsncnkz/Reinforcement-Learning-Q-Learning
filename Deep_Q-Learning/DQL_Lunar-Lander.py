...
class DQLSpaceVehicle:
    ...
    def replay(self,batch_size):
        ...
        if len(SpaceVehicle.memory) < batch_size:
            return
...
if __name__ == "__main__":
    ...
    SpaceVehicle = DQLSpaceVehicle(env)
    ...
    for e in range(episodes):
        ...
        for time in range(1000):
            ...
            # remember / storage
            SpaceVehicle.remember(state, action, reward, next_state, done)

            # update state
            state = next_state

            #Perform experience replay if memory length is greater than minibatch length
            SpaceVehicle.replay(batch_size)

            total_reward += reward
            
            if done:
                SpaceVehicle.targetModelUpdate()
                break
        
        # adjust epsilon
        SpaceVehicle.adaptiveEGreedy()
        
        print("Episode: {} Reward: {}".format(e, total_reward))

# %% test
import time
trained_model = SpaceVehicle
state = env.reset()
